...
export default function AvailabilityPage() {
  ...
  const [providesRecurringBooking, setProvidesRecurringBooking] =
    useState(false);
  const [workingHoursPerDay, setWorkingHoursPerDay] = useState(0);

  // Copy/Paste functionality
  const [copiedSlots, setCopiedSlots] = useState<Array<{
    startTime: string;
    endTime: string;
  }> | null>(null);
  ...
  const fetchAvailabilityTemplate = async () => {
    ...
    try {
      setLoading(true);
      const response = await BookingService.getProviderAvailabilityTemplate(
        user.providerId
      );

      if (response.success) {
        // Transform the API response to match our internal format
        const transformedAvailability = response.payload.availabilities.map(
          (day) => ({
            ...day,
        ...
        console.error(
          "Failed to fetch availability template:",
          response.message
        );
  ...
  const copyTimeSlots = (dayId: string) => {
    const day = availability.find((d) => d.id === dayId);
    if (day && day.slots.length > 0) {
      setCopiedSlots([...day.slots]);
      toast.success(
        `Copied ${day.slots.length} time slot${
  ...
  const pasteTimeSlots = (dayId: string) => {
    if (!copiedSlots || copiedSlots.length === 0) {
      toast.error("No time slots copied");
      return;
    }
    ...
    setAvailability((prev) =>
      prev.map((day) =>
        day.id === dayId
          ? {
              ...day,
              slots: [...copiedSlots].sort(
                (a, b) =>
                  timeToMinutes(a.startTime) - timeToMinutes(b.startTime)
              ),
              available: true, // Automatically mark as available when pasting
            }
          : day
      )
    );

    const dayName = availability.find((d) => d.id === dayId)?.day;
    toast.success(
      `Pasted ${copiedSlots.length} time slot${
        copiedSlots.length > 1 ? "s" : ""
      } to ${dayName}`
    );
  };
  ...
  const selectedDayData = selectedDay
    ? availability.find((day) => day.id === selectedDay)
    : null;
  ...
  const hasChanges =
    ...
    providesRecurringBooking !==
      originalGlobalSettings.providesRecurringBooking ||
      ...
      <div className="mb-6 flex justify-end">
        <button
          ...
          className={`px-6 py-3 rounded-lg font-medium flex items-center transition-colors ${
            hasChanges && !saving && user?.providerId
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setWorkingHoursPerDay(parseInt(e.target.value) || 0)
              }
              className="px-4 py-3 border border-gray-300 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-[#00C2CB] focus:border-transparent transition-all"
              min="0"
              max="24"
          ...
          {selectedDayData ? (
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-semibold text-gray-800">
                    {selectedDayData.day} Schedule
                  </h2>
                  <p className="text-gray-600 text-sm">
                    Manage your availability for {selectedDayData.day}
                  </p>
                </div>
              ...
              <div className="flex items-center space-x-3 mb-6 pb-4 border-b border-gray-200">
                <button
                  onClick={() => copyTimeSlots(selectedDay!)}
                  disabled={
                  ...
                  title="Copy time slots from this day"
                >
                  <svg
                ...
                <button
                  onClick={() => pasteTimeSlots(selectedDay!)}
                  disabled={!copiedSlots || copiedSlots.length === 0}
                  className="flex items-center px-4 py-2 bg-[#00C2CB] text-white rounded-lg hover:bg-[#00b0b9] transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-300"
                  title="Paste copied time slots to this day"
                >
                  <svg
                    className="h-4 w-4 mr-2"
                    fill="none"
              ...
              {selectedDayData.available ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-gray-600 mb-4 max-w-md">
                    You are currently marked as unavailable on{" "}
                    {selectedDayData.day}s. Enable availability to add time
                    slots.
                  </p>
                  <button
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 flex flex-col items-center justify-center h-full min-h-[400px]">
              ...
              <p className="text-gray-600 text-center max-w-md">
                Select a day from the weekly schedule to view and edit time
                slots.
              </p>
            </div>
          )}
