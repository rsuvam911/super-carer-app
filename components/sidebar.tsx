...
export default function Sidebar() {
  ...
  const homeLinks: NavItem[] = [
    { href: "/dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ["client", "provider"] },
    { href: "/clients", label: "Clients", icon: Users, roles: ["provider"] },
    { href: "/bookings", label: "Booking", icon: Calendar, roles: ["client", "provider"] },
    { href: "/invoices", label: "Invoices", icon: FileText, roles: ["client", "provider"] },
    { href: "/chats", label: "Chats", icon: MessageSquare, roles: ["client", "provider"] },
  ]

  const managementLinks: NavItem[] = [
    { href: "/availability", label: "Availability", icon: Clock, roles: ["provider"] },
    { href: "/payment", label: "Payment", icon: CreditCard, roles: ["client", "provider"] },
    { href: "/ratings", label: "Ratings & Reviews", icon: Star, roles: ["provider"] },
  ]

  const systemLinks: NavItem[] = [
    { href: "/settings", label: "Settings", icon: Settings, roles: ["client", "provider"] },
  ]

  const filterByRole = (items: NavItem[]) => items.filter(i => i.roles.includes(roleKey))
  const isActive = (relPath: string) => pathname.startsWith(`${base}${relPath}`)
    ...
    <div className="w-64 h-full bg-[#1a2352] text-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col">
            ...
            {filterByRole(homeLinks).map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={fullHref}
                  className={`flex items-center px-3 py-2 rounded-md text-sm ${active ? "bg-[#00C2CB]/20 text-[#00C2CB]" : "text-gray-300 hover:bg-[#2a3366]"}`}
                >
        ...
        <div className="px-3 py-2 mt-6">
          ...
          <nav className="space-y-1">
            {filterByRole(managementLinks).map(({ href, label, icon: Icon }) => {
              const fullHref = `${base}${href}`
              const active = isActive(href)
              return (
                <Link
